...
test("GitHub Pages edition keeps every article categorized and dated", async () => {
  ...
  const articles = [...html.matchAll(/<article class="story-card" data-group="([^"]+)" data-date="([^"]+)"/g)];
  ...
  for (const [, group, date] of articles) {
  ...
  assert.match(html, /const archiveDate =/);
  assert.match(html, /\[archiveDate\]\.forEach/);
